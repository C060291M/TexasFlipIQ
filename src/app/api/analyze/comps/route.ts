...
export async function GET(req: NextRequest) {
        ...
        if (results.length > 0) {
          const comps = results
            ...
            .map((r: any, i: number) => {
              ...
              return {
                address:       (r.streetAddress || r.address || `${1000+i*200} Unknown St`) + ', ' + (r.city || city || 'TX'),
                price:         typeof price === 'string' ? parseInt(price.replace(/[^0-9]/g,'')) || 0 : price,
  ...
  return NextResponse.json({ comps, source: 'market_estimate' });
}
