...
export const REGIONAL_PRICING: RegionalPricing[] = [
  ...
  { region: 'dfw', regionLabel: 'DFW Metroplex', zipPrefixes: ['750','751','752','753','754','755','756','757','758','759'], laborMultiplier: 1.20, materialMultiplier: 1.10, lastUpdated: '2025-01-15', dataSource: 'TX Contractors Association' },
  ...
  { region: 'east_texas', regionLabel: 'East Texas', zipPrefixes: ['756','757','758','759'], laborMultiplier: 0.96, materialMultiplier: 1.02, lastUpdated: '2025-01-15', dataSource: 'TX Contractors Association' },
  { region: 'west_texas', regionLabel: 'West Texas', zipPrefixes: ['790','791','792','793','794','795','796','797','798','799'], laborMultiplier: 0.92, materialMultiplier: 1.06, lastUpdated: '2025-01-15', dataSource: 'TX Contractors Association' },
...
export function getRegionalPricing(zipCode: string): RegionalPricing {
  ...
  return { region:'other', regionLabel:'Texas', zipPrefixes:[], laborMultiplier:1.00, materialMultiplier:1.00, lastUpdated:'2025-01-15', dataSource:'TX average' };
}
...
export function calculateRehab(input: PropertyInput): RehabResult {
  ...
  return {
    lineItems, total: Math.round(total), perSqft: Math.round(total / sqft),
    region: regional.region, regionLabel: regional.regionLabel,
    laborMultiplier: regional.laborMultiplier, ageMultiplier: ageMult,
    strategyMultiplier: exitStrategy === 'flip' ? 1.18 : exitStrategy === 'str' ? 1.30 : 0.88,
    finishLevel, pricingVersion: 'Q1-2025',
  };
}

export function getBudgetAllocationTargets(strategy: string): Record<string, number> {
  const allocations: Record<string, Record<string, number>> = {
    flip: { Kitchen:28, Bathrooms:20, Flooring:14, 'Roof/Structural':10, 'HVAC/Mech':8, Paint:7, Landscaping:7, Windows:3, Doors:3 },
    str:  { Kitchen:26, Bathrooms:18, 'Furnishing/Decor':16, Flooring:12, 'Outdoor/Amenities':10, Paint:8, 'HVAC/Electrical':6, Landscaping:4 },
    ltr:  { Flooring:22, 'HVAC/Roof':22, Kitchen:18, Bathrooms:15, Electrical:8, Paint:8, Plumbing:4, Landscaping:3 },
  };
