...
export function PropertySidebar({ input, onUpdate }: Props) {
  ...
  const t = (f: keyof PropertyInput) => (e: React.ChangeEvent<HTMLInputElement>) =>
    onUpdate(f, e.target.value);
  ...
  const sec: React.CSSProperties = {
    fontSize: 10, textTransform: 'uppercase',
    letterSpacing: '0.08em', color: '#BF5700',
    fontWeight: 700, borderTop: '1px solid #222',
    paddingTop: 10, marginTop: 12, display: 'block',
  };

  return (
    <div style={{ width: 272, minWidth: 272, borderRight: '1px solid #222', overflowY: 'auto', background: '#111', padding: 16 }}>
        ...
        {([['flip','🏠','Fix & Flip'],['str','✈','STR'],['ltr','📋','LTR']] as [ExitStrategy,string,string][]).map(([id,icon,label]) => (
          <button key={id} onClick={() => onUpdate('exitStrategy', id)}
            style={{
              ...
              border: `1px solid ${input.exitStrategy === id ? '#BF5700' : '#333'}`,
              borderRadius: 8, fontSize: 11, cursor: 'pointer',
              textAlign: 'center',
              background: input.exitStrategy === id ? '#2a1500' : '#1a1a1a',
              color: input.exitStrategy === id ? '#BF5700' : '#888',
              lineHeight: 1.3,
            }}>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        <input
          ...
          value={input.address ?? ''}
          onChange={t('address')}
          placeholder="e.g. 4821 Crestwood Ln"
          style={box}
        />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
        <div>
          ...
          <input type="text" value={input.city ?? ''} onChange={t('city')} placeholder="Austin" style={box} />
        </div>
        <div>
      ...
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#888', marginTop: 8, cursor: 'pointer' }}>
      ...
      {/* Loan preview */}
      <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 8, padding: 10, marginTop: 10, fontSize: 11, color: '#888' }}>
        {[['Loan amount', `$${loan.toLocaleString()}`], ['Monthly interest', `$${moInt.toLocaleString()}`], ['Origination fee', `$${pts.toLocaleString()}`]].map(([l, v]) => (
          <div key={l} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span>{l}</span>
            <span style={{ fontWeight: 600, color: '#BF5700' }}>{v}</span>
          </div>
        ))}
      </div>
