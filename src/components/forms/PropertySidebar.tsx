...
export function PropertySidebar({ input, onUpdate }: Props) {
  ...
  // 70% of ARV only — quick reference, does NOT subtract rehab
  const seventyPctArv = input.arv > 0 ? Math.round(input.arv * 0.70) : 0;
    ...
    <div style={{ width: 272, minWidth: 272, borderRight: '1px solid #DDE3EC', overflowY: 'auto', background: '#FFFFFF', padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
        <div>
          ...
          <input type="text" value={input.city ?? ''} onChange={e => onUpdate('city', e.target.value)} placeholder="Austin" style={box} />
        </div>
        <div>
      ...
      {input.isWaterfront && (
        <div style={{ marginTop: 8, background:'#e8f4f8', border:'1px solid #2980b9', borderRadius:6, padding:'7px 10px', fontSize:11, color:'#1a6080' }}>
          🌊 Waterfront premium applied to comp estimates
        </div>
      )}
      {input.hasPool && (
        <div style={{ marginTop: 6, background:'#e8faf9', border:'1px solid #2EC4B6', borderRadius:6, padding:'7px 10px', fontSize:11, color:'#1a8a82' }}>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        {input.arv === 0 && (
          <div style={{ fontSize: 10, color: '#2EC4B6', marginTop: 3 }}>
            Go to Comps tab and pull comps to auto-set ARV
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        {/* 70% of ARV — quick reference note */}
        {seventyPctArv > 0 && (
          <div style={{ marginTop: 6, background:'#F5F6F8', border:'1px solid #DDE3EC', borderRadius:6, padding:'8px 10px' }}>
            <div style={{ fontSize:10, fontWeight:700, color:'#6B7C93', textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:4 }}>
              70% Rule (quick reference)
            </div>
            <div style={{ fontSize:12, color:'#1F3A5F', marginBottom:4 }}>
              70% of ARV = <strong style={{ fontFamily:'monospace' }}>${seventyPctArv.toLocaleString()}</strong>
            </div>
            <div style={{ fontSize:10, color:'#9ca3af', lineHeight:1.5 }}>
              This is 70% of ARV before subtracting rehab costs.
              Your true MAO (shown in Deal Overview) deducts rehab
              from this number. Use this as a quick ceiling for your offer.
            </div>
            <button
              onClick={() => onUpdate('purchasePrice', seventyPctArv)}
              style={{ marginTop:6, fontSize:10, padding:'3px 10px', background:'#1F3A5F', color:'#fff', border:'none', borderRadius:4, cursor:'pointer', fontWeight:600 }}>
              Use 70% of ARV
            </button>
          </div>
        )}
