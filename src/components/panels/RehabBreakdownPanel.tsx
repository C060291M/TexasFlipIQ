...
export function RehabBreakdownPanel({ input, rehab, enabledItems, onToggle, onSetEnabled }: Props) {
      ...
      {savedAmount > 0 && (
        <div style={{ background:'#e8faf9', border:'1px solid #2EC4B6', borderRadius:10, padding:'12px 16px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ fontSize:13 }}>
            ...
            <span style={{ color:'#1F3A5F' }}>Saving <strong>{fmt(savedAmount)}</strong> by zeroing {zeroedItems.length} item{zeroedItems.length!==1?'s':''}. <strong>Profit waterfall and deal score are updated automatically.</strong></span>
          </div>
          <button
            onClick={() => onSetEnabled(Object.fromEntries(allKeys.map(k => [k, true])))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:10 }}>
            <button onClick={() => onSetEnabled(Object.fromEntries(allKeys.map(k => [k, true])))}
            ...
            <button onClick={() => {
              ...
              onSetEnabled(Object.fromEntries(allKeys.map(k => [k, cosmetic.includes(k)])));
            }}
              style={{ flex:1, padding:'7px', border:'1px solid #DDE3EC', borderRadius:6, background:'#fff', color:'#1F3A5F', cursor:'pointer', fontSize:12, fontWeight:600 }}>
              🎨 Cosmetic only
            </button>
            <button onClick={() => {
              ...
              onSetEnabled(Object.fromEntries(allKeys.map(k => [k, structural.includes(k)])));
            }}
              style={{ flex:1, padding:'7px', border:'1px solid #DDE3EC', borderRadius:6, background:'#fff', color:'#1F3A5F', cursor:'pointer', fontSize:12, fontWeight:600 }}>
              🏗 Structural only
            </button>
          ...
          <div style={{ background:'#FFFFFF', border:'1px solid #DDE3EC', borderRadius:10, overflow:'hidden', boxShadow:'0 1px 3px rgba(31,58,95,0.06)' }}>
              ...
              return (
                <div key={key}
                  ...
                  <span style={{ fontFamily:'monospace', fontSize:13, color:isOn?'#1F3A5F':'#9ca3af', marginRight:12 }}>
                    {isOn ? fmt(val) : <span style={{ textDecoration:'line-through', color:'#ccc' }}>{fmt(val)}</span>}
                  </span>
            ...
            <div style={{ display:'flex', alignItems:'center', padding:'12px 16px', background:'#1F3A5F' }}>
              ...
              <span style={{ flex:1, fontSize:13, fontWeight:700, color:'#fff' }}>
                Adjusted total
                {savedAmount > 0 && <span style={{ fontSize:11, fontWeight:400, color:'#A8BFDA', marginLeft:8 }}>({fmt(savedAmount)} removed)</span>}
              </span>
            ...
            {allEntries.map(([key, val], i) => {
              ...
              return (
                <div key={key} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10, opacity:isOn?1:0.3 }}>
                  ...
                  <span style={{ fontSize:11, fontFamily:'monospace', width:52, color:'#1F3A5F', flexShrink:0 }}>{isOn?fmt(val):'—'}</span>
                </div>
              );
            })}
          </div>
