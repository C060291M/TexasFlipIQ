...
function Card({
  ...
  return (
    <div style={{ background:'#FFFFFF', border:'1px solid #DDE3EC', borderRadius:10, padding:'14px 16px', boxShadow:'0 1px 3px rgba(31,58,95,0.06)' }}>
      <div style={{ fontSize:10, color:'#6B7C93', marginBottom:4, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.05em' }}>
        {label}
      </div>
...
function WaterfallRow({
  ...
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'9px 14px', borderRadius:6, fontSize:13, ...rowStyle[type] }}>
      <span>{label}</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
        {isFlip ? (
          <>
            ...
            <Card label="Max offer (MAO)"   value={fmt(mao)}                             sub="70% rule" />
            <Card label="Total project"     value={fmt(deal.flip?.totalProjectCost ?? 0)} sub="All-in cost" />
          </>
        ) : (
          <>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        <div>
          <div style={{ fontSize:12, fontWeight:700, color:'#1F3A5F', marginBottom:10, textTransform:'uppercase', letterSpacing:'0.08em', display:'flex', alignItems:'center', gap:8 }}>
            Profit waterfall
            {rehabAdj && (
              <span style={{ fontSize:10, fontWeight:600, color:'#2EC4B6', background:'#e8faf9', padding:'2px 8px', borderRadius:10 }}>
                Scope adjusted
              </span>
            )}
          </div>
          ...
          {/* MAO quick-apply box — only show for flip when purchase price > MAO */}
          {isFlip && mao > 0 && input.purchasePrice > mao && onUpdatePurchasePrice && (
            <div style={{ marginTop:10, background:'#fef5e7', border:'1px solid #E07B2A', borderRadius:8, padding:'12px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
              <div style={{ fontSize:12, color:'#78350f', lineHeight:1.5 }}>
                <strong>Purchase price exceeds MAO.</strong><br />
                Max allowable offer at 70% rule: <strong style={{ fontFamily:'monospace' }}>{fmt(mao)}</strong>
              </div>
              <button
                onClick={() => onUpdatePurchasePrice(mao)}
                style={{ flexShrink:0, padding:'8px 16px', background:'#E07B2A', color:'#fff', border:'none', borderRadius:6, fontSize:12, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap' }}>
                Use MAO {fmt(mao)}
              </button>
            </div>
          )}

          {/* Also show when purchase price = 0 and ARV is set */}
          {isFlip && mao > 0 && input.purchasePrice === 0 && onUpdatePurchasePrice && (
            <div style={{ marginTop:10, background:'#e8faf9', border:'1px solid #2EC4B6', borderRadius:8, padding:'12px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
              <div style={{ fontSize:12, color:'#1a8a82', lineHeight:1.5 }}>
                <strong>70% rule MAO calculated.</strong><br />
                Suggested max offer: <strong style={{ fontFamily:'monospace' }}>{fmt(mao)}</strong>
              </div>
              <button
                onClick={() => onUpdatePurchasePrice(mao)}
                style={{ flexShrink:0, padding:'8px 16px', background:'#2EC4B6', color:'#fff', border:'none', borderRadius:6, fontSize:12, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap' }}>
                Use MAO {fmt(mao)}
              </button>
            </div>
          )}
        ...
        <div>
          ...
          {risks.map(r => {
            // Add Use MAO button to the purchase price risk flag
            const isMaoFlag = r.id === 'flip-70-rule' || r.id === 'flip-70-rule-violation';
